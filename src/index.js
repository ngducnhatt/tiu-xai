...
// Fail-fast nếu thiếu ENV
validateEnv();

client.once(Events.ClientReady, async (c) => {
  console.log(`Logged in as ${c.user.tag}`);
  c.user.setPresence({
    activities: [{ name: "/bet | Lợi thế nhà cái = 0", type: ActivityType.Playing }],
    status: "online",
  });
...
client.on(Events.InteractionCreate, async (interaction) => {
  try {
    if (interaction.isChatInputCommand()) {
      ...
      if (["bet", "nap", "rut"].includes(name) && hitCooldown(uid, name, 2000)) {
    ...
    if (interaction.isButton()) {
      ...
      if (interaction.customId.startsWith("adm_")) {
        ...
        if (!tx) {
          return interaction.reply({
            embeds: [
              makeEmbed({ title: "❗ Không tìm thấy giao dịch", desc: ref, color: "error" }),
            ],
            flags: 64,
          });
        }
        ...
        if (kind === "adm_dep_cfm") {
          if (tx.type !== "deposit")
            return interaction.reply({
              embeds: [makeEmbed({ title: "Sai loại", desc: "Không phải deposit.", color: "warn" })],
              flags: 64,
            });
          if (tx.status !== "PENDING")
            return interaction.reply({
              embeds: [makeEmbed({ title: "Đã xử lý", desc: `Trạng thái: ${tx.status}`, color: "warn" })],
              flags: 64,
            });
          ...
          if (!res.changes)
            return interaction.reply({
              embeds: [makeEmbed({ title: "Đã xử lý trước đó", color: "warn" })],
              flags: 64,
            });
            ...
            embeds: [
              {
                ...
                color: 0x22c55e,
                fields: [
                  { name: "👤 User", value: `<@${tx.discord_id}> (${tx.username})` },
                  { name: "💰 Số tiền", value: `${fmtU(BigInt(tx.amount_u))} USDT`, inline: true },
        ...
        if (kind === "adm_wd_cfm") {
          if (tx.type !== "withdraw")
            return interaction.reply({
              embeds: [makeEmbed({ title: "Sai loại", desc: "Không phải withdraw.", color: "warn" })],
              flags: 64,
            });
          if (tx.status !== "HELD")
            return interaction.reply({
          ...
          await notifyUser(client, tx.discord_id, {
            title: "✅ Rút đã duyệt",
            desc: `-${fmtU(BigInt(tx.amount_u))} USDT (đã thanh toán)\nMã GD: \`${ref}\``,
            color: "ok",
          });
        ...
        if (kind === "adm_wd_rej") {
          ...
          db.transaction(() => {
            q.updateBalU.run({ discord_id: tx.discord_id, delta_u: Number(tx.amount_u) }); // hoàn tiền HOLD
          })();
            ...
            embeds: [
              {
                ...
                fields: [
                  { name: "👤 User", value: `<@${tx.discord_id}> (${tx.username})` },
                  { name: "💰 Hoàn", value: `${fmtU(BigInt(tx.amount_u))} USDT`, inline: true },
                  { name: "🧾 Mã GD", value: ref, inline: true },
          ...
          await notifyUser(client, tx.discord_id, {
            ...
            desc: `Đã hoàn lại ${fmtU(BigInt(tx.amount_u))} USDT\nMã GD: \`${ref}\``,
            color: "warn",
          });
  ...
  try {
    ...
    if (cmd === "napid") {
      ...
      if (!/^\d{10,}$/.test(id) || !Number.isFinite(usdt) || usdt <= 0)
        return msg.reply("Sai cú pháp. Dùng: `-napid <discord_id> <usdt> [note]`");
      ...
      return msg.reply(
        `✅ Cộng ${fmtU(amount_u)} USDT cho <@${id}>. Số dư: **${fmtU(afterU)}**. Ref: ${ref}`
      );
    }
    ...
    if (cmd === "rutid") {
      ...
      const note =
        (force ? noteArr : [forceStr, ...noteArr]).filter(Boolean).join(" ") || "ADMIN-ADJUST";
      if (!/^\d{10,}$/.test(id) || !Number.isFinite(usdt) || usdt <= 0)
        return msg.reply("Sai cú pháp. Dùng: `-rutid <discord_id> <usdt> [force] [note]`");
      ...
      if (!force && balU < amount_u)
        return msg.reply(`❗ Balance: ${fmtU(balU)} < ${fmtU(amount_u)}. Dùng \`true\` nếu muốn force.`);
      ...
      return msg.reply(
        `✅ Trừ ${fmtU(amount_u)} USDT của <@${id}>. Số dư: **${fmtU(afterU)}**. Ref: ${ref}`
      );
    }
  } catch (e) {
