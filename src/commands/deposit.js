...
export async function handleDepositButtons(interaction) {
  ...
  if (interaction.customId === "dep_qr") {
    const modal = new ModalBuilder()
      ...
      .setTitle("Nạp VietQR (nhập VND)");
    const amountVnd = new TextInputBuilder()
      .setCustomId("amount_vnd")
      .setLabel(
        `Số tiền VND (1 USDT = ${CFG.vndPerUsdt.toLocaleString("vi-VN")} VND)`
      )
...
export async function handleDepositModals(interaction) {
  ...
  if (interaction.customId === "modal_dep_qr") {
    const vnd = parseVND(interaction.fields.getTextInputValue("amount_vnd"));
    ...
    await replyEmbed(interaction, {
      ...
      fields: [
        ...
        { name: "Số TK", value: `${CFG.vietqr.accountNo}`, inline: true },
    ...
    await logDeposit(interaction.client, {
      ...
      fields: [
        ...
        {
          name: "💰 Số tiền",
          value: `${usdt.toFixed(2)} USDT (${vnd.toLocaleString("vi-VN")} VND)`,
        },
        { name: "🧾 Mã GD", value: ref },
      ],
      components,
  ...
  if (interaction.customId === "modal_dep_crypto") {
    const usdt = parseUSDT(interaction.fields.getTextInputValue("amount_usdt"));
