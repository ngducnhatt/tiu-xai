...
import { toU, fmtU, parseUSDT } from "../money.js";
        ...
        fields: [
            {
                ...
                value: "Rút **USDT** mạng Polygon (USDT0) trực tiếp hoặc **Ngân hàng**.",
            },
...
export async function handleWithdrawButtons(interaction) {
    ...
    if (interaction.customId === "wd_usdt") {
        ...
        const amount = new TextInputBuilder()
            .setCustomId("amount")
            .setLabel(`Số USDT (≥ ${fmtU(MIN_WITHDRAW_U)})`)
            .setPlaceholder("VD: 15, 15.5")
...
export async function handleWithdrawModals(interaction) {
    ...
    if (interaction.customId === "modal_wd_USDT") {
        ...
        if (balU < amount_u) {
            ...
            return replyEmbed(interaction, {
                ...
                fields: [
                    {
                        name: "Số dư",
                        value: `${fmtU(balU)} USDT`,
                        inline: true,
                    },
                    {
                        name: "Yêu cầu rút",
                        value: `${fmtU(amount_u)} USDT`,
                        inline: true,
                    },
        ...
        const ref = `GD-WD-${interaction.user.id}-${Date.now()}`;
        ...
        const components = [
            new ActionRowBuilder().addComponents(
                new ButtonBuilder()
                    .setCustomId(`adm_wd_cfm:${ref}`)
                    .setLabel("✅ Confirm rút")
                    .setStyle(ButtonStyle.Success),
                new ButtonBuilder()
                    .setCustomId(`adm_wd_rej:${ref}`)
                    .setLabel("⛔ Từ chối")
        ...
        await logWithdraw(interaction.client, {
            ...
            fields: [
                {
                    name: "👤 User",
                    value: `<@${interaction.user.id}> (${username})`,
                },
                { name: "💰 Số tiền", value: `${fmtU(amount_u)} USDT` },
                { name: "📍 Ví", value: wallet },
                { name: "🧾 Mã GD", value: ref },
            ],
            components,
        ...
        await logAudit(
            interaction.client,
            `withdraw_HOLD uid=${interaction.user.id} usdt=${fmtU(
                amount_u
            )} ref=${ref}`
        );
        return;
    ...
    if (interaction.customId === "modal_wd_BANK") {
        ...
        const bankName = interaction.fields
            .getTextInputValue("bank_name")
            ?.trim();
        const bankAcc = interaction.fields
            .getTextInputValue("bank_acc")
            ?.trim();
        const bankOwner = interaction.fields
            .getTextInputValue("bank_owner")
            ?.trim();
