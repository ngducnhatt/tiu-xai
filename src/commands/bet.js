    ...
    try {
        ...
        if (!Number.isFinite(amount) || amount <= 0) {
            return editEmbed(interaction, {
                title: "❗ Lỗi",
                fields: [
                    { name: "📝 Chi tiết", value: "Số tiền không hợp lệ." },
                ],
                color: "error",
            });
        }
        ...
        if (amount_u < MIN_BET_U) {
            return editEmbed(interaction, {
                title: "🚫 Từ chối",
                fields: [
                    { name: "🔽 Tối thiểu", value: `${fmtU(MIN_BET_U)} USDT` },
                ],
                color: "warn",
            });
        }
        ...
        const components = [
            new ActionRowBuilder().addComponents(
                new ButtonBuilder()
                    ...
                    .setURL("https://youtu.be/w2MVVZUEUHg")
            ),
        ];
        ...
        return editEmbed(interaction, {
            ...
            fields: [
                ...
                {
                    name: "🔁 Trả thưởng",
                    value: `${fmtU(net_u)} USDT`,
                    inline: true,
                },
                {
                    name: "💼 Số dư mới",
                    value: `${fmtU(newBalU)} USDT`,
                    inline: true,
                },

                {
                    name: "📜 Luật",
                    value: "**Chữ số cuối** của ID tin nhắn này: `0–4 = xỉu`, `5–9 = tài`.",
                },
            ],
            color: win ? "ok" : "error",
            footer: "🍀 Chúc bạn may mắn",
            components,
        });
    } catch (err) {
